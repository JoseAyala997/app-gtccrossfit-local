			...
			<?php
				foreach($data as $row) {
					...
					if($row["membership_valid_to"] > date("Y-m-d")) {
						...
						echo "<td class='text-center'>please upgrade</td>";
						echo "<td class='text-center'>your plan/membership</td>";
					...
					<td>";
						if(!empty($row['member_tag'])) {
							echo "<span class='label' style='background-color: {$row['member_tag']['color']}'>{$row['member_tag']['name']}</span>";
						} else {
							echo "-";
						}
						echo "</td>
					...
					echo " <a href='{$this->request->base}/GymMember/viewAttendance/{$row['id']}' title='Attendance' class='btn btn-flat btn-default  member_attendance'><i class='fa fa-eye'></i></a>";
					echo " <a href='{$this->request->base}/GymProfile/medicalRecord/{$row['id']}' title='".__('Ver Ficha Médica')."' class='btn btn-flat btn-danger medical-record-btn'><i class='fa fa-plus-square'></i></a>";
					echo "</td>";
					if($session['role_name'] == "administrator") {
					echo "<td>";
