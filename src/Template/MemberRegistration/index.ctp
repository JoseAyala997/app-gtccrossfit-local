...
<section class="content">
	...
	<div class="col-md-12 box box-default">
		...
		$member_class = isset($member_class) ? $member_class : [];
		$member_id = isset($member_id) ? $member_id : "";
		$groups = isset($groups) ? $groups : [];
		
		// Función auxiliar para acceso seguro a elementos de arrays
		function safe_get($array, $key, $default = '') {
			if (!is_array($array)) return $default;
			return isset($array[$key]) ? $array[$key] : $default;
		}
		
		// Función segura para json_decode en assign_group
...
echo @$this->Form->select("assign_group", $groups, [
		"default" => safe_json_decode(safe_get($data, 'assign_group')), 
		"multiple" => "multiple", 
		"class" => "form-control group_list"
	]);
echo "</div>";
echo "</div>";
echo "</fieldset>";
...
echo '<div class="col-md-6">';
echo @$this->Form->select("intrested_area", $interest, [
    "default" => safe_get($data, 'intrested_area'), 
    "empty" => __("Select Interest"), 
    "class" => "form-control interest_list"
]);
...
echo '<div class="col-md-6">';
echo @$this->Form->select("g_source", $source, [
    "default" => safe_get($data, 'source'), 
    "empty" => __("Select Source"), 
    "class" => "form-control source_list"
]);
...
echo '<div class="col-md-6">';
echo @$this->Form->select("selected_membership", $membership, [
    "default" => safe_get($data, 'selected_membership'), 
    "empty" => __("Select Membership"), 
...
echo $this->Form->input("", [
    ...
    "class" => "form-control validate[required] mem_valid_from",
    "value" => (
        $edit && !empty($data['membership_valid_from'])
            ? $this->Gym->get_db_format(date($this->Gym->getSettings("date_format"), strtotime($data['membership_valid_from'])))
            : date($this->Gym->getSettings("date_format"))
    )
]);
echo "</div>";
// echo '<div class="col-md-2">';
