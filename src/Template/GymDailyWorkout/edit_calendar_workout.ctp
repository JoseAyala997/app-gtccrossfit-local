        ...
        <div class="inner-container">
            <div class="row">
                <div class="col-md-12">
                    ...
                    <?= $this->Form->create($workout, ['id' => 'edit_wod', 'novalidate' => 'novalidate']) ?>

                    <!-- SECCIÓN PRINCIPAL -->
                    <div class="row">
                        <!-- Columna izquierda: Programa, Fecha, Visibilidad, Notas -->
                        <div class="col-md-6">
                            <h4 class="section-title">Editar Información del Workout</h4>

                            <!-- ID oculto del workout -->
                            <?= $this->Form->hidden('id') ?>
                                ...
                                <div id="visibility-time-container" class="form-group time optional wod_visibility_time visibility-group">
                                    ...
                                    <div>
                                        <?= $this->Form->control('visibility_time', [
                                            ...
                                            'type' => 'time',
                                            'required' => false,
                                            'value' => $workout->visibility_time ? $workout->visibility_time->format('H:i') : null
                                        ]) ?>
                                    </div>
                            ...
                            <div class="form-group">
                                ...
                                <trix-editor toolbar="my_toolbar" input="wod_description"><?= $workout->description ?></trix-editor>
                            ...
                            <div class="form-group">
                                ...
                                <trix-editor toolbar="my_toolbar_coach" input="wod_coach_notes"><?= $workout->coach_notes ?></trix-editor>
                            </div>
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="component-section">
                                ...
                                <div id="components-container">
                                    <!-- Aquí se cargarán los componentes existentes -->
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <a class="btn btn-danger" href="<?= $this->Url->build(['action' => 'calendarioWorkout']) ?>">Cancelar</a>
                    </div>
                    <?= $this->Form->end() ?>
                </div>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Inicializar con la fecha del workout
        const workoutDate = new Date('<?= $workout->date->format('Y-m-d') ?>');

        workoutDateInput.value = formatDisplayDate(workoutDate);
        hiddenDateInput.value = formatServerDate(workoutDate);
        formDateInput.value = formatServerDate(workoutDate);
        ...
        let componentIndex = <?= count($workout->components) ?>; // Iniciar con el número de componentes existentes
        ...
        function createComponent(categoryId, categoryName, existingData = null) {
            ...
            const idField = existingData ? existingData.id : '';
            const activityIdSelected = existingData ? existingData.activity_id : '';
            const descriptionValue = existingData ? existingData.description : '';
            const rxChecked = existingData && existingData.rx ? 'checked' : '';
            ...
            // Campo oculto para ID si es un componente existente
            if (existingData && existingData.id) {
                const idInput = document.createElement('input');
                idInput.type = 'hidden';
                idInput.name = `components[${componentIndex}][id]`;
                idInput.value = existingData.id;
                bodyDiv.appendChild(idInput);
            }
            ...
            filteredActivities.forEach(activity => {
                ...
                if (activityIdSelected == activity.id) {
            ...
            trixInput.value = descriptionValue;
            ...
            // Si tiene contenido existente, establecerlo en el editor
            if (descriptionValue) {
                // Establecer el valor después de que el editor esté listo
                setTimeout(() => {
                    const editor = document.querySelector(`trix-editor[input="${trixInputId}"]`);
                    if (editor) {
                        editor.editor.loadHTML(descriptionValue);
                    }
                }, 100);
            }
        ...
        <?php foreach ($workout->components as $component): ?>
            createComponent(
                ...
                '<?= h($categories[$component->category_id]) ?>', // Agregado h() para escapar HTML
                {
                    id: '<?= $component->id ?>',
                    activity_id: '<?= $component->activity_id ?>',
                    description: <?= json_encode($component->description) ?>,
                    rx: <?= $component->rx ? 'true' : 'false' ?>
                }
            );
        <?php endforeach; ?>
        ...
        function validateForm(event) {
            ...
            if (!formDateInput.value) {
                ...
                // Solo validar la fecha si es diferente a la fecha original
                const originalDate = '<?= $workout->date->format('Y-m-d') ?>';
                if (formDateInput.value !== originalDate && occupiedDates.includes(formDateInput.value)) {
                    isValid = false;
            ...
            if (isValid) {
                document.getElementById('edit_wod').submit();
            } else {
        ...
        document.getElementById('edit_wod').addEventListener('submit', validateForm);
    });
</script>