...

{
	...
	return $week=array('Sunday'=>__('Sunday'),'Monday'=>__('Monday'),'Tuesday'=>__('Tuesday'),'Wednesday'=>__('Wednesday'),'Thursday'=>__('Thursday'),'Friday'=>__('Friday'),'Saturday'=>__('Saturday'));
...
public function get_membership_paymentstatus($mp_id)
{
    ...
    // Consulta SQL directa para verificar si el miembro está en modo mantenimiento
    $maintenance_query = "SELECT 
                           COALESCE(gm.is_maintenance_mode, 0) as is_maintenance_mode,
                           m.maintenance_fee
                         FROM membership_payment mp
                         JOIN gym_member gm ON mp.member_id = gm.id
                         JOIN membership m ON mp.membership_id = m.id
                         WHERE mp.mp_id = :mp_id";
    
    $params = ['mp_id' => $mp_id];
    $maintenance_data = $connection->execute($maintenance_query, $params)->fetch('assoc');
    
    // Determinar si está en modo mantenimiento
    $isMaintenanceMode = ($maintenance_data['is_maintenance_mode'] == 1 );
    $maintenanceFee = $maintenance_data['maintenance_fee'] ?? 0;
    
    // Si está en modo mantenimiento y hay una tarifa de mantenimiento, usar esa tarifa
    if ($isMaintenanceMode && $maintenanceFee > 0) {
        $status = '';
        if ($result['paid_amount'] >= $maintenanceFee) {
        ...
        Log::write('debug', 'Status (Normal Mode): ' . $status);
        return $status;
    }
}
// public function get_membership_paymentstatus($mp_id)
...

{
			...
			switch ($datepicker_lang) {
				...
				case 'vi':
					...
					case 'gu':
						...
						$replace = array('જાન્યુઆરી', 'ફેબ્રુઆરી', 'માર્ચ', 'એપ્રિલ', 'મે', 'જૂન', 'જુલાઈ', 'ઓગસ્ટ', 'સપ્ટેમ્બર', 'ઓક્ટોબર', 'નવેમ્બર', 'ડિસેમ્બર');
	...
	/**
	 * Get the duration value from membership length
	 * 
	 * @param int $membershipLength Membership length in days
	 * @return int Duration value
	 */
	public function getDurationValue($membershipLength)
	{
		if ($membershipLength % 30 === 0) {
			return $membershipLength / 30;
		}
		return $membershipLength;
	}

	/**
	 * Get the duration type from membership length
	 * 
	 * @param int $membershipLength Membership length in days
	 * @return string Duration type ('months' or 'days')
	 */
	public function getDurationType($membershipLength)
	{
		return ($membershipLength % 30 === 0) ? 'months' : 'days';
	}
