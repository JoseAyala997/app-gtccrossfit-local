...

{
...

	{
		...
		if(!empty($results)){
			...
			{			
				...
				$wn = $retrieved_data['workout_name'];
				...
				echo "<div class='col-sm-10'>
					...
					<span class='col-md-2'><input type='text' class='my-workouts validate[required,custom[onlyNumberSp],maxSize[3]]' id='sets' name='sets_".$retrieved_data['id']."' width='50px' value='".((!empty($dd_array)) ? $dd_array[$wn]['sets'] : '')."'></span>

					<span class='col-md-2'><input type='text' class='my-workouts validate[required,custom[onlyNumberSp],maxSize[3]]' id='reps' name='reps_".$retrieved_data['id']."' width='50px' value='".((!empty($dd_array)) ? $dd_array[$wn]['reps'] : '')."'></span>

					<span class='col-md-2'><input type='text' class='my-workouts validate[required,custom[onlyNumberSp],maxSize[3]]' id='kg' name='kg_".$retrieved_data['id']."' width='50px' value='".((!empty($dd_array)) ? $dd_array[$wn]['kg'] : '')."'></span>

					<span class='col-md-2'><input type='text' class='my-workouts validate[required,custom[onlyNumberSp],maxSize[3]]' id='rest' name='rest_".$retrieved_data['id']."' width='50px' value='".((!empty($dd_array)) ? $dd_array[$wn]['rest_time'] : '')."'></span>
			...
			<div class='row'>
				...
				<div class='col-md-12'>
					...
					<?php
						...
						$activities = $activities->leftjoin(
                            ["Activity" => "activity"],
                            ["Activity.id = Membership_Activity.activity_id"]
                        )->select($activity)->hydrate(false)->toArray();
...

					{
						...
						{					
							...
							echo "<div id='activity_{$activity['Activity']['id']}' class='col-md-12 activity_row'>	
								...
								<span class='col-md-2'><input type='text' class='my-workouts validate[required]' id='sets' name='sets_{$activity['Activity']['id']}' width='50px'></span>

								<span class='col-md-2'><input type='text' class='my-workouts validate[required]' id='reps' name='reps_{$activity['Activity']['id']}' width='50px'></span>

								<span class='col-md-2'><input type='text' class='my-workouts validate[required]' id='kg' name='kg_{$activity['Activity']['id']}' width='50px'></span>

								<span class='col-md-3'><input type='text' class='my-workouts validate[required]' id='rest' name='rest_{$activity['Activity']['id']}' width='50px'></span>
	...
	public function gymPay($mp_id, $due) {
		...
		include_once(dirname(__FILE__) . '/GymPaymentForm.php');
	}
				...
				<tr>
					...
					<td align="<?php echo $float_r;?>" width="24%">
						...
						<h5><?php $issue_date=$data[0]['created_date']->format("Y-m-d");
							$issue_date= $this->GYMFunction->get_db_format_en_lang(date($this->GYMFunction->getSettings("date_format"),strtotime($issue_date)));
						...
						<?php
							...
							{ ?>
								...
								<tr>
									...
									<td class="text-center"><?php echo $this->GYMFunction->get_db_format_en_lang(date($sys_data[0]['date_format'],strtotime($data[0]["invoice_date"])));?></td>
	...
	/**
	 * Handle payment form submission with receipt photo upload
	 * 
	 * @return \Cake\Http\Response|null
	 */
	public function gmgtMemberAddPayment() {
		if ($this->request->is('post')) {
			...
			// Initialize receipt photo path
			$receipt_photo = null;
			
			// Handle file upload for Efectivo and Transferencia payment methods
			if (($payment_method === 'Efectivo' || $payment_method === 'Transferencia') && 
				!empty($this->request->data['receipt_photo']['name'])) {
				...
				if ($file_error === 0) {
					...
					$upload_dir = WWW_ROOT . 'upload' . DS . 'receipt_photos';
					...
					if (in_array(strtolower($file_ext), $allowed_exts)) {
						$new_file_name = 'receipt_' . $mp_id . '_' . date('YmdHis') . '.' . $file_ext;
						$upload_path = $upload_dir . DS . $new_file_name;
						
						// Move uploaded file
						if (move_uploaded_file($file_tmp_name, $upload_path)) {
							// Set receipt photo path to save in database
							$receipt_photo = 'upload/receipt_photos/' . $new_file_name;
						} else {
							// File upload failed
							echo json_encode(['status' => false, 'message' => __('Failed to upload receipt photo')]);
							return;
						}
					} else {
						// Invalid file extension
						echo json_encode(['status' => false, 'message' => __('Invalid file format. Allowed formats: jpg, jpeg, png, gif')]);
						return;
					}
				} else {
					// File upload error
					echo json_encode(['status' => false, 'message' => __('Error uploading file')]);
					return;
				}
			}
			
			// Save payment data to database
			$membership_payment_table = TableRegistry::get('MembershipPayment');
			$membership_payment_history_table = TableRegistry::get('MembershipPaymentHistory');
			
			// Get membership payment record
			$membership_payment = $membership_payment_table->get($mp_id);
			
			// Create payment history record
			$payment_history = $membership_payment_history_table->newEntity();
			$payment_history->mp_id = $mp_id;
			$payment_history->amount = $amount;
			$payment_history->payment_method = $payment_method;
			$payment_history->paid_by_date = date('Y-m-d');
			$payment_history->created_by = $created_by;
			$payment_history->receipt_photo = $receipt_photo; // Save receipt photo path
			
			// Set confirmation status based on payment method
			if ($payment_method === 'Efectivo' || $payment_method === 'Transferencia') {
				$payment_history->payment_confirmation_status = 'Pending';
			} else {
				// Other payment methods are automatically confirmed
				$payment_history->payment_confirmation_status = 'Confirmed';
				$payment_history->confirmed_by = $session['id'];
				$payment_history->confirmed_date = date('Y-m-d H:i:s');
			}
			
			if ($membership_payment_history_table->save($payment_history)) {
				// Update membership payment record
				$membership_payment->paid_amount = $membership_payment->paid_amount + $amount;
				
				// Update payment status
				if ($membership_payment->paid_amount >= $membership_payment->membership_amount) {
					$membership_payment->payment_status = 'Paid';
				} else {
					$membership_payment->payment_status = 'Partially Paid';
				}
				
				if ($membership_payment_table->save($membership_payment)) {
					echo json_encode(['status' => true, 'message' => __('Payment added successfully')]);
				} else {
					echo json_encode(['status' => false, 'message' => __('Failed to update payment status')]);
				}
			} else {
				echo json_encode(['status' => false, 'message' => __('Failed to save payment')]);
			}
		}
	}
}