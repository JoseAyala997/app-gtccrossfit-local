...
{  
    ...
    {		
		...
		if($session["role_name"] == "staff_member")
		{
			...
			{
				$data = $this->GymAssignWorkout->find("all")
					->contain(["GymMember"])
					->where(["GymMember.assign_staff_mem" => $session["id"], "member_type" => "Member"])
					->select([
						'GymAssignWorkout.user_id',
						'latest_id' => $this->GymAssignWorkout->find()->func()->max('GymAssignWorkout.id'),
						'GymMember.first_name',
						'GymMember.last_name',
						'GymMember.image',
						'GymMember.member_id',
						'GymMember.intrested_area'
					])
					->group(['GymAssignWorkout.user_id', 'GymMember.first_name', 'GymMember.last_name', 'GymMember.image', 'GymMember.member_id', 'GymMember.intrested_area'])
					->hydrate(false)
					->toArray();
			}
			else
